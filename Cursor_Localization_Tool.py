...
def ZhuRu_QuWei_Spinner_DongCi():
    ...
    # 2. 注入 workbench.desktop.main.js
    DesktopLuJing = os.path.join(AppMuLu, "out", "vs", "workbench", "workbench.desktop.main.js")
    if os.path.isfile(DesktopLuJing):
        try:
            ...
            if re.search(Pattern, NeiRong):
                NeiRong = re.sub(Pattern, HelperJs, NeiRong)
                GengXin = True
            elif 'function _fs(e){' in NeiRong:
                NeiRong = NeiRong.replace('function _fs(e){', HelperJs + 'function _fs(e){', 1)
                GengXin = True
            ...
            for Yuan, MuBiao in TiHuanDuiDesktop:
                if Yuan in NeiRong:
                    NeiRong = NeiRong.replace(Yuan, MuBiao, 1)
                    GengXin = True
...
def HuiFu_QuWei_Spinner_DongCi():
    ...
    if os.path.isfile(GlassLuJing):
        try:
            ...
            for MuBiao, Yuan in HuanYuanDui:
    ...
    # 2. 恢复 workbench.desktop.main.js
    DesktopLuJing = os.path.join(AppMuLu, "out", "vs", "workbench", "workbench.desktop.main.js")
    if os.path.isfile(DesktopLuJing):
        try:
            NeiRong, HuanHang = DuQu_WenBen_BaoLiu_HuanHang(DesktopLuJing)
            ...
            for MuBiao, Yuan in HuanYuanDuiDesktop:
                if MuBiao in NeiRong:
                    NeiRong = NeiRong.replace(MuBiao, Yuan, 1)
                    GengXin = True
